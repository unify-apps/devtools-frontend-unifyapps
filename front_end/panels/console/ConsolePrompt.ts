...
const str_ = i18n.i18n.registerUIStrings(
  "panels/console/ConsolePrompt.ts",
  UIStrings
);
...
>(UI.Widget.Widget) {
  ...
  constructor() {
    ...
    this.historyInternal =
      new TextEditor.AutocompleteHistory.AutocompleteHistory(
    ...
    this.innerPreviewElement = this.eagerPreviewElement.createChild(
      "div",
      "console-eager-inner-preview"
    );
    const previewIcon = new IconButton.Icon.Icon();
    previewIcon.data = {
      iconName: "chevron-left-dot",
    ...
    const editorContainerElement = this.element.createChild(
      "div",
    ...
    this.eagerEvalSetting.addChangeListener(
      this.eagerSettingChanged.bind(this)
    );
    this.eagerPreviewElement.classList.toggle(
      "hidden",
      !this.eagerEvalSetting.get()
    );
    ...
    const extensions = [
      ...
      CodeMirror.javascript.javascriptLanguage.data.of({
        autocomplete: (
          context: CodeMirror.CompletionContext
        ): CodeMirror.CompletionResult | null =>
          this.addCompletionsFromHistory
            ? this.#editorHistory.historyCompletions(context)
            : null,
      }),
      CodeMirror.EditorView.contentAttributes.of({
        "aria-label": i18nString(UIStrings.consolePrompt),
      }),
    ...
    this.#editorHistory = new TextEditor.TextEditorHistory.TextEditorHistory(
      this.editor,
      this.historyInternal
    );
  ...
  private onTextChanged(): void {
    ...
    if (this.eagerEvalSetting.get()) {
      const asSoonAsPossible = !TextEditor.Config.contentIncludingHint(
        this.editor.editor
      );
  ...
  private async requestPreview(): Promise<void> {
    ...
    const text = TextEditor.Config.contentIncludingHint(
      this.editor.editor
    ).trim();
    const executionContext = UI.Context.Context.instance().flavor(
      SDK.RuntimeModel.ExecutionContext
    );
    const { preview, result } =
      await ObjectUI.JavaScriptREPL.JavaScriptREPL.evaluateAndBuildPreview(
        text,
    ...
    if (
      preview.deepTextContent() !==
      TextEditor.Config.contentIncludingHint(this.editor.editor).trim()
    ) {
  ...
  private isCaretAtEndOfPrompt(): boolean {
    return (
      this.editor.state.selection.main.head === this.editor.state.doc.length
    );
  }

  moveCaretToEndOfPrompt(): void {
    this.editor.dispatch({
      selection: CodeMirror.EditorSelection.cursor(
  ...
  private editorKeymap(): readonly CodeMirror.KeyBinding[] {
    return [
      {
        key: "ArrowUp",
        run: (): boolean => this.#editorHistory.moveHistory(Direction.BACKWARD),
      },
      {
        key: "ArrowDown",
        run: (): boolean => this.#editorHistory.moveHistory(Direction.FORWARD),
      },
      {
        mac: "Ctrl-p",
        run: (): boolean =>
          this.#editorHistory.moveHistory(Direction.BACKWARD, true),
      },
      {
        mac: "Ctrl-n",
        run: (): boolean =>
          this.#editorHistory.moveHistory(Direction.FORWARD, true),
      },
      {
        key: "Escape",
        run: (): boolean => {
          return TextEditor.JavaScript.closeArgumentsHintsTooltip(
            this.editor.editor,
            this.#argumentHintsState
          );
        },
      },
      {
  ...
  private async enterWillEvaluate(forceEvaluate?: boolean): Promise<boolean> {
    ...
    const isExpressionComplete =
      await TextEditor.JavaScript.isExpressionComplete(doc.toString());
    if (
  ...
  showSelfXssWarning(): void {
    Common.Console.Console.instance().warn(
      i18nString(UIStrings.selfXssWarning, {
        PH1: i18nString(UIStrings.allowPasting),
      })
    );
    this.#selfXssWarningShown = true;
    Host.userMetrics.actionTaken(
      Host.UserMetrics.Action.SelfXssWarningConsoleMessageShown
    );
  }

  private async handleEnter(forceEvaluate?: boolean): Promise<void> {
    if (
      this.#selfXssWarningShown &&
      ...
      Host.userMetrics.actionTaken(
        Host.UserMetrics.Action.SelfXssAllowPastingInConsole
      );
      return;
    }
    ...
    if (await this.enterWillEvaluate(forceEvaluate)) {
      ...
      TextEditor.JavaScript.closeArgumentsHintsTooltip(
        this.editor.editor,
        this.#argumentHintsState
      );
      this.editor.dispatch({
  ...
  private updatePromptIcon(): void {
    void this.iconThrottler.schedule(async () => {
      this.promptIcon.classList.toggle(
        "console-prompt-incomplete",
        !(await this.enterWillEvaluate())
      );
    });
  }
    ...
    if (currentExecutionContext) {
      ...
      if (consoleModel) {
        ...
        if (ConsolePanel.instance().isShowing()) {
          Host.userMetrics.actionTaken(
            Host.UserMetrics.Action.CommandEvaluatedInConsolePanel
          );
        }
      }
    }
  ...
  private async evaluateCommandInConsole(
    ...
    if (
      ...
      if (callFrame) {
        const nameMap =
          await SourceMapScopes.NamesResolver.allVariablesInCallFrame(
            callFrame
          );
  ...
  private async substituteNames(
    expression: string,
    mapping: Map<string, string | null>
  ): Promise<string> {
    try {
      return await Formatter.FormatterWorkerPool.formatterWorkerPool().javaScriptSubstitute(
        expression,
        mapping
      );
    } catch {
  ...
  private editorUpdate(update: CodeMirror.ViewUpdate): void {
    if (
      update.docChanged ||
      CodeMirror.selectedCompletion(update.state) !==
        CodeMirror.selectedCompletion(update.startState)
    ) {
      this.onTextChanged();
